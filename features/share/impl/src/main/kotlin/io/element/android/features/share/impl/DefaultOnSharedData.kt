    ...
    override fun invoke(data: ShareIntentData) {
        when (data) {
            ...
            is ShareIntentData.Uris -> {
                // Never delete an incoming URI based only on its authority. The sender controls
                // the URI text and could otherwise make SecureChat delete a private provider file
                // using SecureChat's own process privileges.
