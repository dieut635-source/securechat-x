...
class SecureChatPolicyPublisherTest {
    ...
    fun `a failure to publish is reported and does not throw`() {
        ...
        val gateway = FakeGateway(applyResult = Result.failure(IllegalStateException("binder died")))
    ...
    fun `the build-time source publishes exactly the three agreed keys`() {
        ...
        SecureChatPolicyPublisher(SecureChatDeviceOwner(gateway), BuildTimePolicySource()).publish()
    ...
    fun `the build-time source does not drift from the app's own defaults`() {
        ...
        assertThat(BuildTimePolicySource().currentPolicy()).isEqualTo(MdmConfig.default)
    }
}
